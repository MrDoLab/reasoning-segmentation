...
from src.config import FINAL_CONFIG, IMAGE_SIZE
...
def run_final(
    ...
    """
    Final submission pipeline (full test set, FINAL_CONFIG 고정)
    ...
    unload_mllm = FINAL_CONFIG['unload_mllm']
    ...
    if verbose:
        print("=" * 60)
        print("v4 Final Submission Pipeline")
        print("=" * 60)
        print(f"samples={n}, single={n - multi_count}, multi={multi_count}")

    stats = {'single': 0, 'multi': 0}

    for item in tqdm(all_data, desc="Final submission", disable=not verbose):
        query = item['query']
    ...
    if verbose:
        print(f"\nFinal submission completed: single={stats['single']}, multi={stats['multi']}")
